...
def count_files(path: Path, suffixes: set[str] | None = None) -> int:
    ...
    for item in path.iterdir():
        if not item.is_file() or item.name.lower().startswith("readme"):
            continue
...
def get_dashboard_data() -> dict[str, Any]:
    ...
    heartbeat = parse_dt(runtime_state.get("heartbeat_at"))
    heartbeat_age = None
    ...
    watcher_status = str(runtime_state.get("status") or "not_started")
    ...
    return {
        ...
        "system": {
            ...
            "watcher_status": runtime_state.get("status") or "not_started",
            "watcher_heartbeat_at": runtime_state.get("heartbeat_at"),
            ...
            "watcher_project_root": runtime_state.get(
                "project_root", str(root)
            ),
        },
        "paths": {
...
def execute_action(action: str) -> dict[str, Any]:
    ...
    runtime_state = read_runtime_state(root)
    watcher_status = str(runtime_state.get("status") or "not_started")
    watcher_ready = watcher_status == "running"
