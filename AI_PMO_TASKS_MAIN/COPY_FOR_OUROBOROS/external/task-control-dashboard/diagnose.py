...
try:
    ...
    heartbeat_path = root / "archive" / ".local_runtime_heartbeat.json"
    state_path = root / "archive" / ".local_runtime_state.json"
    result["heartbeat_file"] = str(heartbeat_path)
    result["heartbeat_exists"] = heartbeat_path.exists()
    result["runtime_state_file"] = str(state_path)
    result["runtime_state_exists"] = state_path.exists()
    if heartbeat_path.exists():
        try:
            result["heartbeat"] = json.loads(
                heartbeat_path.read_text(encoding="utf-8")
            )
        except Exception as exc:
            result["heartbeat_read_error"] = str(exc)
except Exception as exc:
