...
HEARTBEAT_CONTEXT: dict[str, Any] = {
    "status": "starting",
    "phase": "starting",
    "details": "Запуск companion-процесса",
}
...
def write_heartbeat(root: Path) -> None:
    ...
    with HEARTBEAT_LOCK:
        payload = dict(HEARTBEAT_CONTEXT)
    payload.update(
        {
...
def heartbeat_loop(root: Path) -> None:
...
def start_heartbeat(root: Path) -> threading.Thread:
    HEARTBEAT_STOP.clear()
    set_heartbeat_phase(
        "starting",
        details="Companion-процесс запускается",
    )
    write_heartbeat(root)
    thread = threading.Thread(
        target=heartbeat_loop,
        args=(root,),
        name="ai-pmo-heartbeat",
        daemon=True,
    )
    thread.start()
    return thread
...
def current_inputs(root: Path) -> list[tuple[str, Path]]:
    ...
    for kind, folder in (
        ...
        for path in sorted(folder.iterdir()):
            if (
                not is_ignored_input_file(path)
...
def process_command_queue(
    ...
    for command_path in queued_commands(root)[:10]:
        try:
            ...
            set_heartbeat_phase(
                "processing_command",
                details=f"Выполняется команда: {action}",
            ...
            try:
                ...
                set_heartbeat_phase(
                    "idle",
                    details="Ожидание новых файлов, команд или расписания",
                )
...
def run_watcher() -> int:
    ...
    heartbeat_thread = start_heartbeat(root)
    ...
    while not STOP:
        ...
        try:
            ...
            with HEARTBEAT_LOCK:
                current_phase = str(
                    HEARTBEAT_CONTEXT.get("phase") or ""
                )
            if current_phase == "recovering":
                set_heartbeat_phase(
                    "idle",
                    details="Ожидание новых файлов, команд или расписания",
                )
            ...
            if files_allowed:
                ...
                if changed:
                    set_heartbeat_phase(
                        "processing_files",
                        details="Обрабатываются новые входные файлы",
            ...
            if (
                ...
                set_heartbeat_phase(
                    "processing_schedule",
                    details="Выполняется плановый полный контроль",
    ...
    set_heartbeat_phase(
        "stopped",
        status="stopped",
        details="Companion-процесс остановлен",
    )
    try:
        write_heartbeat(root)
    except Exception:
